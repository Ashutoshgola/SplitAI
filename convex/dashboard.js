...
export const getUserBalances = query({
  handler: async (ctx) => {
    ...
    const expenses = (await ctx.db.query("expenses").collect()).filter(
      (e) =>
        !e.groupId && // 1‑to‑1 pay of the users
...
// This is used if we want to sort our amount
...
export const getTotalSpent = query({
  handler: async (ctx) => {
    ...
    const expenses = await ctx.db
      .query("expenses")
      .withIndex("by_date", (q) => q.gte("date", startOfYear)) //gte is greater than or equal to 
      .collect();
